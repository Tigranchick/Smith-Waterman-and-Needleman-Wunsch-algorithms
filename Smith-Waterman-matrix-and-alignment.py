...
def blosum_62(alpha, beta):
    if alpha == beta:
        if (alpha, beta) in blosum62:
            return blosum62[(alpha, beta)]
        else:
            return blosum62[(beta, alpha)]
    elif alpha == '-' or beta == '-':
        return pt['gap']
    else:
        if (alpha, beta) in blosum62:
...
def SmithWaterman(s1, s2, scorematrice):

    '''
    Score matrix of alignment
    s1, s2 - protein sequences
    scorematrice - choose between mch, PAM_250, blosum_62
    mch represents 'match': 1, 'mismatch': -2, 'gap': -2
    '''
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            diag = score[i - 1][j - 1] + scorematrice(s1[i - 1], s2[j - 1])
    ...
    while i > 0 and j > 0:
        ...
        if score_current == score_diag + scorematrice(s1[i - 1], s2[j - 1]):
            print('diag')
            a1, a2 = s1[i - 1], s2[j - 1]
            i, j = i - 1, j - 1
